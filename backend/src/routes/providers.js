...
router.get("/nearby", async (req, res) => {
  ...
  try {
    ...
    const providers = await pool.request().query(
      "SELECT p.Id, p.Name, p.Phone, p.LastLat, p.LastLng, p.BaseFee, p.PerKmFee, p.IsOnline FROM Providers p WHERE p.IsOnline = 1 AND p.LastLat IS NOT NULL AND p.LastLng IS NOT NULL"
    );
